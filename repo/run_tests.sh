...
# ── 1. Boot Docker ──────────────────────────────────────────────────
# Zero-Config-File security model: pgAdmin generates its own admin
# credentials inside the container at startup (see
# docker/pgadmin/Dockerfile + docker/pgadmin/entrypoint.sh). Plain
# `docker compose up` works on any machine with no shell exports,
# no .env file, and no helper script. The credentials show up in
# `docker compose logs pgadmin` once the stack is healthy.
