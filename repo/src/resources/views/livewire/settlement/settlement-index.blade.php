<div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-surface-200 overflow-x-auto">
        <table class="w-full text-sm" role="table">
            ...
            <tbody>
                {{-- $settlements is a LengthAwarePaginator wrapping the
                     /api/settlements payload; rows are plain associative
                     arrays from OrderResource-style serialisation. --}}
                @forelse($settlements as $s)
                <tr class="border-b border-surface-100">
                    <td class="px-4 py-3 font-medium">{{ $s['reference'] ?? '' }}</td>
                    <td class="px-4 py-3">{{ !empty($s['period_start']) ? \Illuminate\Support\Carbon::parse($s['period_start'])->format('M d') : '' }} – {{ !empty($s['period_end']) ? \Illuminate\Support\Carbon::parse($s['period_end'])->format('M d, Y') : '' }}</td>
                    <td class="px-4 py-3 text-right">${{ number_format((float)($s['gross_amount'] ?? 0), 2) }}</td>
                    <td class="px-4 py-3 text-right text-red-600">-${{ number_format((float)($s['refund_total'] ?? 0), 2) }}</td>
                    <td class="px-4 py-3 text-right font-bold">${{ number_format((float)($s['net_amount'] ?? 0), 2) }}</td>
                    ...
                    <td class="px-4 py-3">
                        @if($canFinalize && ($s['status'] ?? '') !== 'finalized')
                            <button wire:click="finalize({{ (int)($s['id'] ?? 0) }})" class="text-brand-600 hover:underline text-xs" tabindex="0">Finalize</button>
                        @endif
                    </td>
                </tr>
                @empty
