<div>
    ...
    <?php
        // Button-level gating: the Livewire component fetches the
        // caller's effective permission slugs from /auth/me and the
        // blade conditionally renders controls based on the SAME
        // permission slugs the API middleware enforces. This keeps
        // the UI and the backend in lockstep — no more "button visible
        // but click returns 403" friction, and no role assumptions
        // baked into the view.
        $canGenerate = in_array('settlements.generate', $effectivePermissions, true);
        $canFinalize = in_array('settlements.finalize', $effectivePermissions, true);
    ?>

    
    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($canGenerate): ?>
        ...
        <table class="w-full text-sm" role="table">
            ...
            <tbody>
                ...
                <tr class="border-b border-surface-100">
                    ...
                    <td class="px-4 py-3"><?php echo e(!empty($s['period_start']) ? \Illuminate\Support\Carbon::parse($s['period_start'])->format('M d') : ''); ?> – <?php echo e(!empty($s['period_end']) ? \Illuminate\Support\Carbon::parse($s['period_end'])->format('M d, Y') : ''); ?></td>
                    <td class="px-4 py-3 text-right">$<?php echo e(number_format((float)($s['gross_amount'] ?? 0), 2)); ?></td>
                    <td class="px-4 py-3 text-right text-red-600">-$<?php echo e(number_format((float)($s['refund_total'] ?? 0), 2)); ?></td>
                    <td class="px-4 py-3 text-right font-bold">$<?php echo e(number_format((float)($s['net_amount'] ?? 0), 2)); ?></td>
                    ...
                    <td class="px-4 py-3">
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($canFinalize && ($s['status'] ?? '') !== 'finalized'): ?>
                            <button wire:click="finalize(<?php echo e((int)($s['id'] ?? 0)); ?>)" class="text-brand-600 hover:underline text-xs" tabindex="0">Finalize</button>
                        <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                    </td>
                </tr>
